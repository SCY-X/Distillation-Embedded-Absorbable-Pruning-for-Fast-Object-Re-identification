        ...
        if self.training:
            ...
            for m in self.base.modules():
                if hasattr(m, 'get_lasso_vetor'):
                    conv1_lasso, compactor_lasso = m.get_lasso_vetor()
                    conv1_lasso_list.append(conv1_lasso)
                    conv2_lasso_list.append(compactor_lasso)
                if hasattr(m, 'get_feature_map'):
                    s_map.append(m.get_feature_map())

            for m in self.Teacher.Teacher_model.base.modules():
                if hasattr(m, 'get_feature_map'):
                    t_map.append(m.get_feature_map())
            return [y, teacher_y], global_feat, conv1_lasso_list, conv2_lasso_list, [[s_map, t_map], [stu_feat, teacher_feat]]
        else:
            return feat
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(0):
        ...
        #net.load_param_test('/home/xieyi/SSD/xieyi/Vehicle_Reid/KD/KD_Prune/LKD_Rep_ResNet_2/log/s_resnet50_original_90.pth')
        print("--------------")
        net.eval()
        a = time.time()
        result1 = net(x)
